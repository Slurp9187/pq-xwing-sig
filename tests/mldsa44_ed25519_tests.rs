...
fn test_rng() -> ChaCha12Rng {
    ChaCha12Rng::from_seed([42u8; 32]) // Arbitrary seed for determinism
}

#[test]
fn test_generate_keypair() {
    let mut rng = test_rng();
    let (sk, vk): (SigningKey, VerifyingKey) = generate_keypair(&mut rng);
...
fn test_sign_verify_roundtrip() {
    ...
    let sig: Signature = sk.sign(message, context, &mut rng).unwrap();
...
fn test_verify_tampered_signature() {
    ...
    let tampered_sig: Signature = Signature::try_from(&sig_bytes[..]).unwrap();
...
fn test_invalid_context_length() {
    let mut rng = test_rng();
    let (sk, _): (SigningKey, VerifyingKey) = generate_keypair(&mut rng);
    let message = b"Hello, post-quantum world!";
...
fn test_serialization_roundtrip() {
    ...
    // Manual deserialization (adjusted for MLDSA44)
...
fn test_deterministic_keys_from_seed() {
    let seed = [1u8; 32]; // 32 bytes for Ed25519 seed
...
fn test_verify_wrong_key() {
    let mut rng = test_rng();
    let (sk, vk): (SigningKey, VerifyingKey) = generate_keypair(&mut rng);
    let (_, wrong_vk): (SigningKey, VerifyingKey) = generate_keypair(&mut rng);
    let message = b"Hello, post-quantum world!";
    let context = b"test_context";
    let sig: Signature = sk.sign(message, context, &mut rng).unwrap();
    assert!(vk.verify(message, &sig, context).is_ok());
...
// TODO: Add draft test vectors (Appendix E)
