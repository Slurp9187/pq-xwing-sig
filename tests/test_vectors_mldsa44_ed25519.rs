...
fn test_composite_vectors() {
    ...
    for test in tests {
        ...
        // Filter to only MLDSA44-Ed25519 composite cases
        if !tc_id.contains("MLDSA44-Ed25519") {
        ...
        // Decode sk as raw bytes (expect 64 for composite)
        ...
        // Assert length and convert to fixed array (64 bytes for composite sk)
        assert_eq!(
            sk_bytes.len(),
            64,
        ...
        // Deterministic RNG (all-zeroes for reproducible new sig)
        struct ZeroRng;
